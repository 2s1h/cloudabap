...
*      INTO TABLE @DATA(airports).
    ...
    SELECT
      ...
      RIGHT OUTER JOIN /lrn/airport AS f
      ON c~airport_from_id = f~airport_id
      RIGHT OUTER JOIN /lrn/airport AS t
    ...
    FIELDS c~carrier_id, c~connection_id,
           ...
           tstmp_seconds_between( tstmp1 =
                                  ...
                                  date = @today,
                                  time = c~arrival_time,
                                  tzone = t~timzone ) ) , 60 ) AS duration
...
*              TIME ZONE connection-timzone_to"airports[ airport_id = connection-airport_to_id ]-timzone
...
*currency_conversion( amount => amount, source_currency => source_currency,
*target_currency => target_currency, exchange_rate_date => exchange_rate_date )
