    ...
    LOOP AT connections_buffer INTO DATA(connection).
      ...
      connection-duration = utclong_diff( high = arrival_utclong
        ...
        if connection-duration < 0.
          connection-duration = connection-duration + ( 24 * 60 ).
        endif.
                                          MODIFY connections_buffer FROM connection TRANSPORTING duration.
*                connection-duration = "utclong_diff( departure_utclong - arrival_utclong.
    ENDLOOP.
  ENDMETHOD.
...
CLASS lcl_carrier IMPLEMENTATION.
  ...
  METHOD get_output.
    ...
    APPEND | { text-001 }  { me->name } | TO r_result.
    APPEND | { text-002 }  { lines( passenger_flights ) } | TO r_result.
    APPEND | { text-003 }  { get_average_free_seats(  ) } | TO r_result.
    APPEND | { text-004 }  { lines( cargo_flights     ) } | TO r_result.
